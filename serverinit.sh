...
# ║               by Franklin · franklin-sys.vercel.app             ║
# ╚══════════════════════════════════════════════════════════════════╝
# Usage:
#   Interactive:   sudo bash serverinit.sh
#   CI/Unattended: SI_STACK=1 SI_SEC=2 SI_SSH_PORT=2222 sudo bash serverinit.sh --ci
#
# Remote (download first — required for interactive mode):
...
# ── Smart disk limiter ────────────────────────────────────────────────────────
# Ensure at least 5 GB free disk remains after swap creation.
# Example: 7 GB free, SWAP_GB=4 → 3 GB left — too tight for Docker/Node.
...
# ══════════════════════════════════════════════════════════════════════════════
#  PHASE 1 — CONFIGURATION (interactive or CI)
# ══════════════════════════════════════════════════════════════════════════════

if [[ $CI_MODE -eq 1 ]]; then
  # ── CI mode: read from env vars, validate, set defaults ───────────────────
  ...
  # Q3: SSH port (full security only)
  SSH_PORT=22
...
if [[ $CI_MODE -eq 0 ]]; then
  ...
  info "CI mode: подтверждение пропущено — запуск автоматически"
fi
